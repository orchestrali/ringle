...
function dimensions() {
  ...
  let a = Math.min(Math.floor(e.clientHeight * (8 / 6)), 560);
  let s = 6 * Math.floor(a / 8);
...
function buildanswer() {
  ...
  let changes = Math.max(method.leadLength, 6);
  //console.log(changes);
  ...
  if (changes > 6) {
    let start = Math.floor(Math.random() * (changes-6));
    if (start%2 === 1) start -= 1;
    rowArr = rowArr.slice(start, start+6);
  }
...
function checkGuess() {
  ...
  } else if ([1,2,3,4,5,6,7,8].some(n => !board[rownum].includes(n))) {
...
function evaluate() {
  ...
  if (!board || !rowArr || rownum >= board.length || rownum >= rowArr.length) {
    ...
    } else if ((i > 0 && board[rownum][i] === rowArr[rownum][i-1]) || (i < 7 && board[rownum][i] === rowArr[rownum][i+1])) {
